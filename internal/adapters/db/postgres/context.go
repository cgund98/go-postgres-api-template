...
// DbContext is injected into the context to allow repositories to access the database context without explicit parameters
type DbContext struct {
	Tx pgx.Tx // The transaction
}
...
func GetTXFromContext(ctx context.Context) *DbContext {
	if dbContext, ok := ctx.Value(txKey).(*DbContext); ok {
